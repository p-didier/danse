...
def main(
    ...
    # Load data
    snr, estoi, dnSNRs = load_data(baseExportPath)

    fig = plot_data(snr, estoi, dnSNRs)
    if exportFigure:
...
def load_data(baseExportPath):
    ...
    for ii, folder in enumerate(folders):
        ...
        dnSNRs[ii] = re.findall(r'\d+', folder.name)[0]

    # Order by increasing DN SNR
    idx = np.argsort(dnSNRs)
    dnSNRs = dnSNRs[idx]
    snrDataOut = snrDataOut[:, idx, :]
    estoiDataOut = estoiDataOut[:, idx, :]

    return snrDataOut, estoiDataOut, dnSNRs


def plot_data(snr: np.ndarray, estoi: np.ndarray, dnSNRs: np.ndarray):
    """
    Parameters:
    -----------
    snr: [nNodes x nSNRs x 4] np.ndarray
        SNR data. Reference for last index: [raw, local, centralized, DANSE].
    estoi: [nNodes x nSNRs x 4] np.ndarray
    ...
    for ii in range(snr.shape[0]):
        # Plot SNR
        for jj in range(snr.shape[2]):
            axes[0, ii].plot(dnSNRs, snr[ii, :, jj], f'{symbols[jj]}-', markersize=4)
        axes[0, ii].grid()
        ...
        # Plot eSTOI
        for jj in range(snr.shape[2]):
            axes[1, ii].plot(dnSNRs, estoi[ii, :, jj], f'{symbols[jj]}-', markersize=4)
        axes[1, ii].grid()
        axes[1, ii].set_title(f'Node {ii+1}')
        axes[1, ii].set_xlabel('Diffuse noise (DN) SNR [dB]')
        if ii == 0:
